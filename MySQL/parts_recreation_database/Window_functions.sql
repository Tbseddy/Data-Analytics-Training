...
# Rolling total(Moving total, Running total or Cumulative sum)
-- This is the sum of values that updates as new data becomes available typically over a defined period or sequence.
-- for instance. In a 7-day rolling total for sales, the total for each day includes the sales for that day and the previous 6 days.
-- A rolling total is going to start at a specific value and add on valus from subsequent rows.
...
SELECT dem.employee_id, dem.first_name, gender, salary,
ROW_NUMBER() OVER(PARTITION BY gender ORDER BY salary desc) rownumber,
Rank() OVER(PARTITION BY gender ORDER BY salary desc) rank_number,
dense_rank() OVER(PARTITION BY gender ORDER BY salary desc) denserank_num -- this is numerically ordered instead of positional like rank
FROM employee_demographics dem
JOIN employee_salary sal
	ON dem.employee_id = sal.employee_id
;